        ...
        public void FrameAnalysis(byte mybyte)
        {
            switch (mybyte)
            {
                case (byte)FrameType.UPLINK:
                    ...
                    user2=HamCode.Testing.DeCodingStr(comBuffer2);
                ...
                case (byte)FrameType.DAT:
                    ...
                    if (HamCode.Testing.DeCodingStr(comBuffer) == "") WriteData(null, FrameType.RET, false);//неуспешно приняли просим повторно 
                    else
                    {
                        WriteData(null, FrameType.ACK, false);//успешно приняли отправляем квитанцию
                        string a = HamCode.Testing.DeCodingStr(comBuffer);
                        if (a.Substring(0, 2).Equals("~~"))
                        { a=a.Remove(0,2);
                            user2 = a;
                            //WriteData("~~Kek", CommunicationManager.FrameType.DAT, false);
                            break; }
                        DisplayData(MessageType.Incoming, user2 + " [ " + DateTime.Now + " ] \n" + HamCode.Testing.DeCodingStr(comBuffer) + "\n");
                    }
                    break;
            ...
            switch (CurrentFrameType)
            {
                case FrameType.DAT:
                    try
                    {
                        ...
                        byte[] newMsg = HamCode.Testing.CodingStr(msg).ToArray();
